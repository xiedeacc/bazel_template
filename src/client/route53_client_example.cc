...
class Route53Client {
 public:
  ...
  bool ManageDNSRecord(const std::string& hosted_zone_id,
                       ...
                       int32_t ttl = 300, const std::string& region = "") {
    ...
    if (status.ok()) {
      std::string operation_name =
          (operation == OpCode::OP_ROUTE53_UPDATE_A_RECORD)
              ? "A Record Update"
              : "CNAME Record Update";
      std::cout << operation_name
                << " successful for domain: " << response.domain_name() << '\n';
      std::cout << "Record Type: " << response.record_type() << '\n';
      std::cout << "New Value: " << response.new_value() << '\n';
      std::cout << "Change ID: " << response.change_id() << '\n';
  ...
  bool UpdateARecord(const std::string& hosted_zone_id,
                     const std::string& domain_name,
                     const std::string& new_ip_address, int32_t ttl = 300,
  ...
  bool UpdateCNAMERecord(const std::string& hosted_zone_id,
                         const std::string& domain_name,
                         const std::string& new_domain_name, int32_t ttl = 300,
                         const std::string& region = "") {
